...
def fixed_numbers():
	#define three fixed input values
	A = 1
	B = 0
	Cin = 1
	return A ,B , Cin
...
def Golden_Model(A,B,Cin):
	# golden model for Full Adder
	print("Golden_Modle Input A :",A)
	print("Golden_Modle Input B :",B)
	print("Golden_Modle Input Cin :",Cin)
	S = (A^B)^Cin
	Cout = (A&B) | ((A^B)&Cin)
	return S,Cout
...
async def fa_test(dut):


	#A,B,Carry_in = fixed_numbers()
	for i in range(10):
		A,B,Carry_in = Random_numbers()
		Golden_Model_Sum,Golden_Model_Cout=Golden_Model(A,B,Carry_in)
		...
		#display the Golden Model Output
		print("Golden Model SUM Output:",Golden_Model_Sum)
		print("Golden Model Cout Output:",Golden_Model_Cout)
